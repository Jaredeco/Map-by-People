...
app.config['SQLALCHEMY_BINDS'] = {'two' : 'sqlite:///database/mapcontent.sqlite3', 'three' : 'sqlite:///database/comments.sqlite3'}
...
def create_marker():
	if request.method == "POST":
		...
		new_coor = Map(latitude=lat, longitude=lng, address=ad, content=con, name=nam, likes=0, dislikes=0)
...
def marker_page(id):
	...
	if request.method == "POST":
		...
		db.session.commit()
		return render_template("marker_page.html", item=item, commnts=commnts)
	else:
		return render_template("marker_page.html", item=item,commnts=commnts)
...
def like():
	item = Map.query.filter_by(id=id).first()
	return render_template("marker_page.html", item=item)
...
def m():
	...
	data = {}
	for item in val:
		lati = {'latitude' + str(item.id) : item.latitude, 'longitude' + str(item.id): item.longitude, 'address' + str(item.id): item.address }
		data.update(lati)
